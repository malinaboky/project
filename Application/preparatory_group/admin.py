...
class CognitiveDevelopAdmin(admin.ModelAdmin):
    # TODO Эта модель с ссылками один к одному, поэтому если не указать джанге, не что бы не отображать во всех связанных моделях это поле,
    #  то будет выводиться ссылка на эту модель, а она нахер не нужна.
...
class ArtisticDevelopAdmin(admin.ModelAdmin):
    all_fields = [field.name for field in get_model_field(ArtisticDevelop) if field.name not in ('id', 'artisticdevelop', 'painting', 'application', 'modeling')]
...
class AttentionAndMemoryAdmin(admin.ModelAdmin):
    all_fields = [field.name for field in get_model_field(AttentionAndMemory) if field.name not in ('id', 'artisticdevelop')]
...
class ThinkingAndSpeakingAdmin(admin.ModelAdmin):
    all_fields = [field.name for field in get_model_field(ThinkingAndSpeaking) if field.name not in ('id', 'artisticdevelop')]
...
class EmotionsAndWillAdmin(admin.ModelAdmin):
    all_fields = [field.name for field in get_model_field(EmotionsAndWill) if field.name not in ('id', 'artisticdevelop')]
...
class VisualPerceptionAdmin(admin.ModelAdmin):
    all_fields = [field.name for field in get_model_field(VisualPerception) if field.name not in ('id', 'artisticdevelop')]
